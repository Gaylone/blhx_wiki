...
author: Mythe
这个模块定义了插件的图像处理部分
当前版本只支持建造图像的处理，后续会将舰船信息的图像处理重构加入本模块
感谢Yuri-YuzuChaN的osuv2项目，本模块直接或间接的用到了该项目下的部分代码，项目链接如下
https://github.com/Yuri-YuzuChaN/osuv2

本模块中的PIL库需要用户使用以下语句自行安装，请注意！
pip install pillow
'''

from PIL import Image, ImageDraw, ImageFont
import os
from .AzurlaneAPI import get_ship_data_by_id
import asyncio

# 代码合并时请修改这里的路径！！！！！
PATH = os.path.dirname(__file__)
FONT_GACHA_LV = os.path.join(PATH, 'fonts', 'number.ttf')                   # 抽卡时等级的字体文件
FONT_GACHA_NM = os.path.join(PATH, 'fonts', 'ZhunYuan.otf')                 # 抽卡时下面文字的字体文件
PATH_ROOT_GACHA = os.path.join(PATH, 'gacha_data')                          # 图片资源根目录
PATH_EDGE_GACHA = os.path.join(PATH, 'gacha_data', 'image_frame')           # 外边框目录
PATH_CHAR_GACHA = os.path.join(PATH, 'gacha_data', 'character_image')       # 角色立绘目录
PATH_STAR_GACHA = os.path.join(PATH, 'gacha_data', 'star')                  # 星星图片目录
PATH_TYPE_GACHA = os.path.join(PATH, 'gacha_data', 'type_icon')             # 舰种图片目录
PATH_BG_GACHA = os.path.join(PATH, 'gacha_data', 'level_background')        # 背景图片目录
# -----------------------------------
# -----------------------------------
# -----------------------------------
...
class GachaImage:
    ...
    async def MakeSinglePic(self, info):
        ...
        # 绘制背景
        im_bg = Image.open(os.path.join(PATH_BG_GACHA, info['rarity'] + '.png'))
        im_bg.convert('RGBA')
        DrawImage(im, im_bg, (int((im.size[0] - im_bg.size[0]) / 2), int((im.size[1] - im_bg.size[1]) / 2)))
        # 绘制角色
        try:
            im_ch = Image.open(os.path.join(PATH_CHAR_GACHA, info['id'] + '.png'))
        except:
            im_ch = Image.open(os.path.join(PATH_CHAR_GACHA, 'unknown.png'))
        im_ch.convert('RGBA')
        w, h = im.size
        ...
        # 绘制类型
        im_tp = Image.open(os.path.join(PATH_TYPE_GACHA, info['type'] + '.png'))
        im_tp.convert('RGBA')
        ...
        im = DrawText(im, text, '#FFFFFF')
        # 绘制边框
        im_ed = Image.open(os.path.join(PATH_EDGE_GACHA, info['rarity'] + '.png'))
        w, h = im.size
        if info['rarity'] == 'Ultra Rare':
            w = int(w * 0.99)
            h = int(h * 0.99)
        ...
        # 绘制星星
        im_st = Image.open(os.path.join(PATH_STAR_GACHA, info['rarity'] + '.png'))
    ...
    async def Make(self):
        ...
        im = Image.open(os.path.join(PATH_ROOT_GACHA, 'background.jpg'))
        ...
        savepath = os.path.join(PATH, 'images', 'gacha.jpg')
